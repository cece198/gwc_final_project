...
var count = 0;
	class Bag
	{
			...
			{	
				...
				{
					return false;//bag was not clicked
				}
				if(((x<=xPos + 9 && x>= xPos - 9) && (y<=yPos + 40 && y>= yPos - 40)) || 
					((x<=xPos + 9 && x>= xPos - 9) && (y<=yPos + 80 && y>= yPos - 80)) || 
					((x<=xPos + 40 && x>= xPos - 40) && (y<=yPos + 40 && y>= yPos - 40)))
				{
					count=count+1;
					drawing=false;
					xPos=-500;
